"""
Largely copy paste from https://github.com/facebookresearch/xcit/blob/master/xcit.py
with some small modifications to add xcit_small and retrieve cls token attention
in final layer
...
class ClassAttentionBlock(nn.Module):
    ...
    def forward(self, x, H, W, mask=None, return_attention=False):
...
class XCiT(nn.Module):
    ...
    def get_last_selfattention(self, x):
        ...
        for blk in self.cls_attn_blocks:
            x, a = blk(x, Hp, Wp, return_attention=True)
            attn = a
...
## def xcit_small(pretrained=False, **kwargs):
##     model = XCiT(
##         embed_dim=384, depth=12, num_heads=8, mlp_ratio=4, qkv_bias=True,
...
def xcit_small(pretrained=False, **kwargs):
    model = XCiT(
        embed_dim=384, depth=12, num_heads=16, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), eta=1e-5, tokens_norm=True,
        num_classes=0, **kwargs)
    model.default_cfg = _cfg()
    return model
