"""
Modified from https://github.com/lucidrains/vit-pytorch/blob/main/\
vit_pytorch/dino.py

Modified to allow for:
    - distributed training
    - custom additions to loss function
        - clustering loss
        - expression loss
"""
...
def loss_fn(
    ...
    return - (
        teacher_probs * torch.log(student_probs + eps)
    ).sum(dim=-1).mean()
...
class MLP(nn.Module):
    def __init__(self, dim, dim_out, num_layers, hidden_size=256):
        ...
        for ind, (layer_dim_in, layer_dim_out) in enumerate(zip(dims[:-1],
                                                            dims[1:])):
...
class NetWrapper(nn.Module):
    def __init__(self, net, output_dim, projection_hidden_size,
                 projection_num_layers, layer=-2):
        super().__init__()
        self.net = net
        self.layer = layer
    ...
    def _get_projector(self, hidden):
        _, dim = hidden.shape
        projector = MLP(dim, self.output_dim, self.projection_num_layers,
...
class Dino(nn.Module):
    def __init__(
        ...
        self.local_crop = T.RandomResizedCrop((image_size, image_size),
                                              scale=(0.05,
                                              local_upper_crop_scale))
        self.global_crop = T.RandomResizedCrop((image_size, image_size),
                                               scale=(global_lower_crop_scale,
                                               1.))

        self.student_encoder = NetWrapper(net, num_classes_K,
        ...
        if self.is_distributed:
            self.student_encoder = nn.parallel.DistributedDataParallel(
                self.student_encoder,
                device_ids=[self.gpu]
            )

##         self.teacher_encoder = None  # is set with singleton method
        self.teacher_ema_updater = EMA(moving_average_decay)

        self.register_buffer('teacher_centers', torch.zeros(1, num_classes_K))
        self.register_buffer('last_teacher_centers',
                             torch.zeros(1, num_classes_K))
...
##         return teacher_encoder
    ...
    def update_moving_average(self):
        ...
        # make sure we collect the center from all processes

        update_moving_average(self.teacher_ema_updater, self.teacher_encoder,
                              self.student_encoder)
    ...
    def forward(
        ...
        local_image_one, local_image_two = self.local_crop(
            image_one),  self.local_crop(image_two)
        global_image_one, global_image_two = self.global_crop(
            image_one), self.global_crop(image_two)
        ...
        teacher_logits_avg = torch.cat(
            (teacher_proj_one, teacher_proj_two)
        ).mean(dim=0)
